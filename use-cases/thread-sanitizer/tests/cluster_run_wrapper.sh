...
LOG_DIR="${TMPDIR}/results"
APP_LOG_NAME="${SLURM_ARRAY_JOB_ID}_${SLURM_ARRAY_TASK_ID}"
mkdir -p "$LOG_DIR"
cp "${TARGET_DIR}/timing.csv" "${LOG_DIR}/${APP_LOG_NAME}.csv-acc"

LOG_FILE="${TARGET_DIR}/drb_time.log"
echo "id,threads,mode,time,exit_code" | tee "${LOG_DIR}/${APP_LOG_NAME}.csv-time"
(
    ...
    echo ""
) | tee -a "${LOG_DIR}/${APP_LOG_NAME}.csv-time"
