...
bool AllocTrackerLTOPass::instrument_allocation_calls(
    ...
    for (CallInst* call_I : alloc_calls) {
        ...
        // Retrieve important arguments dependent on which allocation function is passed
        // i.e. for malloc retrieve size argument, for calloc retrieve size and num argument etc.
        ...
        for (int arg_idx : config.arg_indices) {
            ...
            // Note: normal allocation calls use i64 type in LLVM IR for the size parameter (i.e. size_t in the C program)
            //       if someone uses wrapper functions that pass the size as i32 (i.e. int in the C program)
            //       the logging function will have a type mismatch and IR verification will fail.
        ...
        //  i.e. only possible if precalc analysis ignored problematic MPI communication and marked call as to skip
        Value* valid_V = nullptr;
        auto skip_it = std::find(problematic_calls_in_slice_.begin(), problematic_calls_in_slice_.end(), call_I);
        ...
        for (auto* arg: processed_args) args.push_back(arg);
        for (auto* flag: constant_flags) args.push_back(flag);
        ...
        // only keep it if the allocation is actually required to maintain correctness.
        if (!logging::has_call_I_further_uses(call_I)) call_I->eraseFromParent();
...
bool AllocTrackerLTOPass::instrument_MPI_Init_and_MPI_Finalize_if_needed(Module& M, LLVMContext& Ctx) {
    bool modified = false;
    ...
    if (mpi_init_CI && mpi_finalize_CI) {
        ...
        logging::declare_function(M, Ctx, "alloc_tracker_mpi_init_called", FunctionType::get(Type::getVoidTy(Ctx), {}, false));
        logging::declare_function(M, Ctx, "alloc_tracker_mpi_finalize_called", FunctionType::get(Type::getVoidTy(Ctx), {}, false));

        // Instrument both calls with respective instrumentation function from runtime library
        Function* init_instrumentation_F = M.getFunction("alloc_tracker_mpi_init_called");
        Function* finalize_instrumentation_F = M.getFunction("alloc_tracker_mpi_finalize_called");
        ...
        llvm::errs() << "[AllocTrackerLTOPass::instrument_allocations_with_logging_functions] Added instrumentation for MPI_Init() and MPI_Finalize().\n";

        modified = true;
    } else {
        assert((!mpi_init_CI && !mpi_finalize_CI) && "The program should not contain any MPI calls.");
    }
...
bool AllocTrackerLTOPass::instrument_allocations_with_logging_functions(Module& M, ModuleAnalysisManager& MAM) {
    bool modified = false;

    LLVMContext& Ctx = M.getContext();
    logging::declare_logging_functions(M, Ctx);

    // Instrument start and end of program with logging functions (but only if not already done in slice_module)
    if (!sliced_) { 
        if (instrument_start_and_end_of_program(M)) modified = true;
    }

    // Check whether MPI_Init and MPI_Finalize exist and then add instrumentation for logging library
    if (instrument_MPI_Init_and_MPI_Finalize_if_needed(M, Ctx)) modified = true;
        ...
        for (auto& I: llvm::instructions(F)) {
            if (auto* call_I = llvm::dyn_cast<CallInst>(&I)) {
                ...
                if (auto* called_F = call_I->getCalledFunction()) {
                    if (called_F->isIntrinsic()) continue;

                    llvm::StringRef called_F_name = called_F->getName();

                    if (called_F_name == "malloc") malloc_calls.push_back(call_I);
                    else if (called_F_name == "calloc") calloc_calls.push_back(call_I);
                    else if (called_F_name == "realloc") realloc_calls.push_back(call_I);
                    else if (called_F_name == "aligned_alloc") aligned_alloc_calls.push_back(call_I);
                }
            }
        }
    }
    ...
    AllocationInstrumentationConfig malloc_config = { "alloc_tracker_log_malloc_call", { fst_arg_idx } };
    ...
    AllocationInstrumentationConfig calloc_config = { "alloc_tracker_log_calloc_call", { fst_arg_idx, snd_arg_idx } };

    // Configuration for realloc
    // signature: void *realloc( void *ptr, size_t new_size );
    // interested only in second argument (size, index 1)
    AllocationInstrumentationConfig realloc_config = { "alloc_tracker_log_realloc_call", { snd_arg_idx } };

    // Configuration for aligned_alloc
    // signature: void *aligned_alloc( size_t alignment, size_t size );
    // interested only in second argument (size, index 1)
    AllocationInstrumentationConfig aligned_alloc_config = { "alloc_tracker_log_aligned_alloc_call", { snd_arg_idx } };
        

    if (instrument_allocation_calls(M, Ctx, malloc_calls, malloc_config))  modified = true;
    if (instrument_allocation_calls(M, Ctx, calloc_calls, calloc_config))  modified = true;
    if (instrument_allocation_calls(M, Ctx, realloc_calls, realloc_config))  modified = true;
    if (instrument_allocation_calls(M, Ctx, aligned_alloc_calls, aligned_alloc_config))  modified = true;

    llvm::errs() << "[AllocTrackerLTOPass::instrument_allocations_with_logging_functions] Finished instrumenting with logging functions.\n";

    // Verify module
    if (!utils::verify_module(M)) throw std::runtime_error("[AllocTrackerLTOPass::instrument_allocations_with_logging_functions] ERROR: Module could not be verfied!");
...
{
    // Note: Currently, it is only tested to add MPI_{Send, Recv, Bcast} to the slicing criterion.
    //      This can be modified here to also include other MPI functions.
    //      However, these must be then also handled in the precompute library (in precalculations.cpp) 
    //      in the function visit_call_from_ptr().
    std::vector<llvm::StringRef> required_mpi_functions;

    if (AddAllMPICommunication) {
        // In this case we want to add all the MPI_{Send, Recv, Bcast} calls to the slice 
    ...
    bool found = std::find(required_mpi_functions.begin(), required_mpi_functions.end(), called_F_name) != required_mpi_functions.end();
...
void AllocTrackerLTOPass::set_problematic_call_args_to(int new_arg_value) {
    llvm::errs() << "[AllocTrackerLTOPass] Setting the arguments of the problematic allocation calls found during slicing analysis to " << new_arg_value << "\n";
        ...
        if (auto* call_I = llvm::dyn_cast<CallInst>(I)) {
            if (auto* called_F = call_I->getCalledFunction()) {
                ...
                if (called_F_name == "malloc") {
                    Value* malloc_size_V = call_I->getArgOperand(0); // size is first argument

                    auto* zero_size_CI = llvm::ConstantInt::get(malloc_size_V->getType(), new_arg_value);

                    call_I->setArgOperand(0, zero_size_CI);
                }
                else if (called_F_name == "calloc") {
                    Value* calloc_num_V = call_I->getArgOperand(0); // num is first argument
                    Value* calloc_size_V = call_I->getArgOperand(1); // size is second argument

                    auto* zero_num_CI = llvm::ConstantInt::get(calloc_num_V->getType(), new_arg_value);
                    auto* zero_size_CI = llvm::ConstantInt::get(calloc_size_V->getType(), new_arg_value);

                    call_I->setArgOperand(0, zero_num_CI);
                    call_I->setArgOperand(1, zero_size_CI);
                }
                else if (called_F_name == "realloc") {
                    Value* realloc_size_V = call_I->getArgOperand(1); // new size is second argument

                    auto* zero_size_CI = llvm::ConstantInt::get(realloc_size_V->getType(), new_arg_value);

                    call_I->setArgOperand(1, zero_size_CI);
                }
                else if (called_F_name == "aligned_alloc") {
                    Value* aligned_alloc_alignment_V = call_I->getArgOperand(0); // alignment is first argument
                    Value* aligned_alloc_size_V = call_I->getArgOperand(1); // size is second argument

                    auto* zero_alignment_CI = llvm::ConstantInt::get(aligned_alloc_alignment_V->getType(), new_arg_value);
                    auto* zero_size_CI = llvm::ConstantInt::get(aligned_alloc_size_V->getType(), new_arg_value);
...
bool AllocTrackerLTOPass::slice_module(Module& M, ModuleAnalysisManager& MAM) {
    ...
    Function* main_F = M.getFunction("main");
    if (!main_F) {
        llvm::errs() << "[AllocTrackerLTOPass] Aborting: Could not find main function.\n";
        return false;
    }

    // combined slicing criterion <S, V>
    std::vector<Value *> to_precompute; // these are the values v in set V
    std::vector<Instruction *> precompute_locations; // these are the locations s in set S

    // Traverse all instructions within the function and find call instructions that should be added to slicing criterion
    for (Function& F: M) {
        if (F.isDeclaration()) continue;
        ...
        for (auto& I: llvm::instructions(F)) {
            if (auto* call_I = llvm::dyn_cast<CallInst>(&I)) {
                ...
                if (auto* called_F = call_I->getCalledFunction()) {
                    ...
                    // Check if current call instruction is a required MPI function call, then add it to the slicing criterion.
                    // This is only needed if AddAllMPICommunication is set
                    add_required_mpi_function_to_slicing_criterion(to_precompute, precompute_locations, call_I, called_F_name);
    ...
    llvm::errs() << "to_precompute size = " << to_precompute.size() << ", precompute_locations size = " << precompute_locations.size() << "\n";

    // Nothing to do
    if (precompute_locations.empty()) {
        llvm::errs() << "[AllocTrackerLTOPass] Note: No dynamic memory allocation functions found. Abort Slicing.\n";
        return false;
    }
    ...
    if (mpi_init_call) {
        ...
        assert(mpi_init_call->arg_size() == 2 && "MPI Init does not have two arguments."); 
    ...
    precalc_analysis_ = std::make_shared<PrecalculationAnalysis>(
        M, main_F, to_precompute, precompute_locations, problematic_calls_, IgnoreMPICommunication, AddAllMPICommunication
    );

    // Remove duplicates from problematic allocation call vector (are sometimes added in slicing analysis)
    utils::remove_duplicates_from_vector(problematic_calls_);

    // Actually slice the module
    precalculation_ = std::make_shared<PrecomputeInsertion>(M, precalc_analysis_, false);

    // Retrieve and save all Instructions from problematic_calls (that are problematic allocations) in the new slice
    llvm::errs() << "[AllocTrackerLTOPass] Instructions / dynamic allocations to skip:\n";
    for (auto* e: problematic_calls_) {
        problematic_calls_in_slice_.push_back(precalculation_->get_precomputed_value(e));
    }

    // Set all the arguments of the problematic allocation calls to 0.
    // This ensures no problematic call that still needs to be kept in slice will actually allocate uninitialized memory,
    // i.e. random values most likely very large numbers that would let the allocation fail.
    ...
    //++it; // second instruction is NOT call to precomputed main since we removed some precompute specific functions
    ...
    // only for evaluation: calls runtime function that logs program start timestamp
    ...
    // afterwards if MPI calls are still presenent. Need to do that for every return?
    if (input_program_uses_mpi_ && argc_ptr && argv_ptr) {
        llvm::errs() << "[AllocTrackerLTOPass] Adding MPI Init again after slicing.\n";

        Function* mpi_init_F = M.getFunction("MPI_Init");
        assert(mpi_init_F && "Could not find MPI_Init() function declaration after slicing!");
    ...
    if (input_program_uses_mpi_ && argc_ptr && argv_ptr) {
        llvm::errs() << "[AllocTrackerLTOPass] Adding MPI Finalize again after slicing.\n";

        Function* mpi_finalize_F = M.getFunction("MPI_Finalize");
        assert(mpi_finalize_F && "Could not find MPI_Finalize() function declaration after slicing!");
    ...
    if (!utils::verify_module(M)) throw std::runtime_error("[AllocTrackerLTOPass] ERROR: Module could not be verfied!");

    // run analysis passes like dead code elim to remove unnecessary function copies 
...
bool AllocTrackerLTOPass::inline_allocation_wrapper_call(Module& M, CallInst* call_I) {
    // Only inline valid call instruction that is not marked no inline
    if (!call_I) return false;
        
    Function* called_F = call_I->getCalledFunction();

    // Only proceed if called function exists and is defined
    if (!called_F || called_F->isDeclaration()) return false;

    // Remove attributes optnon and noinline if present
    if (called_F->hasFnAttribute(Attribute::OptimizeNone)) called_F->removeFnAttr(Attribute::OptimizeNone);
    if (called_F->hasFnAttribute(Attribute::NoInline)) called_F->removeFnAttr(Attribute::NoInline);
    ...
    if (!IR.isSuccess()) {
        llvm::errs() << "[AllocTrackerLTOPass] Failed to inline function: " << called_F->getName() << "\n";
        return false;
    }
...
bool AllocTrackerLTOPass::is_allocation_wrapper(llvm::StringRef name) const {
    if (AllocationWrapperFunctions.empty()) return false;
        
    auto it = std::find(
        AllocationWrapperFunctions.begin(),
        AllocationWrapperFunctions.end(),
        name.str()
    );      
...
bool AllocTrackerLTOPass::inline_allocation_wrapper_calls(Module& M) {
    ...
    llvm::errs() << "[AllocTrackerLTOPass] Inlining all wrapper function calls for dynamic memory allocation functions ...\n";

    bool modified = false;
    unsigned int n_inlined = 0;

    // Two phase approach to avoid invalidating iterators when iterating over module
    // and transforming module at the same time
    ...
    llvm::errs() << "[AllocTrackerLTOPass] Finished inlining " << n_inlined << " wrapper function calls in " << inline_duration << " seconds.\n";
...
bool AllocTrackerLTOPass::run_on_module(Module& M, ModuleAnalysisManager& MAM) {
    ...
    bool replaced = EnableLogging ? instrument_allocations_with_logging_functions(M, MAM) : false;

    double module_total_duration = module_timer.stop();
    llvm::errs() << "\n[AllocTrackerLTOPass] Time to run AllocTrackerLTOPass: " << module_total_duration << " seconds\n\n";
...
PreservedAnalyses AllocTrackerLTOPass::run(Module& M, ModuleAnalysisManager& MAM) {
    llvm::errs() << "[AllocTrackerLTOPass] Running pass on Module with name: " << M.getName() << "\n";
