...
void declare_function(Module &M, LLVMContext &Ctx, const std::string &func_name,
                      FunctionType *func_Ty) {
  if (M.getFunction(func_name))
    return;
...
void declare_logging_functions(Module &M, LLVMContext &Ctx) {
  ...
  declare_function(
      M, Ctx, "alloc_tracker_log_calloc_call",
  ...
  declare_function(
      M, Ctx, "alloc_tracker_log_realloc_call",
      FunctionType::get(PointerType::getUnqual(Ctx),
                        {
                            Type::getInt64Ty(Ctx),       // size
                            Type::getInt1Ty(Ctx),        // is_constant_size
  ...
  declare_function(
      M, Ctx, "alloc_tracker_log_aligned_alloc_call",
      FunctionType::get(PointerType::getUnqual(Ctx),
                        {
                            Type::getInt64Ty(Ctx),       // size
                            Type::getInt1Ty(Ctx),        // is_constant_size
...
GlobalVariable *get_or_create_global_string_constant(Module &M,
                                                     LLVMContext &Ctx,
                                                     std::string str) {
  ...
  Constant *instruction_string_global_C =
      ConstantDataArray::getString(Ctx, str);
  return new GlobalVariable(M, instruction_string_global_C->getType(),
                            true, // isConstant
                            GlobalValue::PrivateLinkage,
                            instruction_string_global_C, global_var_name);
}

GlobalVariable *get_parent_function_name_V(Module &M, LLVMContext &Ctx,
                                           Function &F) {
  ...
  for (auto &GV : M.globals()) {
    if (GV.getName().starts_with(global_var_name)) {
      if (GV.isConstant() && GV.hasInitializer()) {
        if (auto *CDA = dyn_cast<ConstantDataArray>(GV.getInitializer())) {
          if (CDA->isString() &&
              CDA->getAsString().rtrim('\0') == parent_func_name) {
            return &GV;
          }
        }
      }
  ...
  Constant *parent_func_name_array =
      ConstantDataArray::getString(Ctx, parent_func_name);
  return new GlobalVariable(M, parent_func_name_array->getType(),
                            true, // isConstant
                            GlobalValue::PrivateLinkage, parent_func_name_array,
...
void instrument_start_of_program(Module &M, IRBuilder<> &builder) {
  ...
  logging::declare_function(M, Ctx, "alloc_tracker_program_start",
                            FunctionType::get(Type::getVoidTy(Ctx), {}, false));
  Function *program_start_instrumentation_F =
      M.getFunction("alloc_tracker_program_start");
...
void instrument_end_of_program(Module &M, IRBuilder<> &builder) {
  ...
  logging::declare_function(M, Ctx, "alloc_tracker_program_end",
                            FunctionType::get(Type::getVoidTy(Ctx), {}, false));
  Function *program_end_instrumentation_F =
      M.getFunction("alloc_tracker_program_end");
