...
void run_optimization_passes(llvm::Module& M, ModuleAnalysisManager& MAM) {
    ...
    llvm::errs() << "[AllocTrackerLTOPass] Run (global) Dead Code Elimination Pass\n";
    // Removes unused functions
    //  i.e. functions from original module that are now replaced with slice functions
...
/**
 * This struct is used in the instrumnentation functions to instrument different allocation calls.
 * Since these functions have different types and number of arguments this config stores the indices
 * to the important arguments and allows a cleaner implementation without code duplication.
 */
...
public:
    ...
    bool instrument_MPI_Init_and_MPI_Finalize_if_needed(Module& M, LLVMContext& Ctx);
    bool instrument_allocation_calls(
        Module& M, 
        LLVMContext& Ctx,
    ...
    bool instrument_allocations_with_logging_functions(Module& M, ModuleAnalysisManager& MAM);
...
extern "C" LLVM_ATTRIBUTE_WEAK PassPluginLibraryInfo llvmGetPassPluginInfo() {
  return {
      ...
      [](PassBuilder &PB) {
        ...
        //  - registerFullLinkTimeOptimizationLastEPCallback: pass is run after other optimizations and thus views a potentially highly transformed code due to different optimizations
        PB.registerFullLinkTimeOptimizationEarlyEPCallback([&](ModulePassManager& MPM, OptimizationLevel Level) {
            MPM.addPass(AllocTrackerLTOPass());
        });

        // needed only for manual invokation of pass by name (within the opt command)?
        // i.e. for testing/debugging without optimizations applied etc.
