/*
This file was taken from https://github.com/apommel/3d-heat-diffusion-mpi
...
void solveMPI(int n, double *u0, double r, int nt, int argc, char **argv)
{
    ...
    free(sub_u0);
    free(sub_u1);
    free(i_pos_send); free(i_pos_recv);
    free(i_neg_send); free(i_neg_recv);
    free(j_pos_send); free(j_pos_recv);
    free(j_neg_send); free(j_neg_recv);
    free(k_pos_send); free(k_pos_recv);
    free(k_neg_send); free(k_neg_recv);
