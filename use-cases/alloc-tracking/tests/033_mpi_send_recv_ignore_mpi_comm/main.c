...
int main(int argc, char **argv) {
    ...
    if (rank == 0) {
        ...
        // test calloc dependent on mpi recv
        int* p = (int*)calloc(sizeof(double), recv_size + i);
        printf("p from calloc: %p\n", p);
        free(p);

        // test realloc dependent on mpi recv
        size_t old_size = recv_size * sizeof(int);
        int* pre = (int*)malloc(old_size); 
        int* ppre = realloc(pre, old_size * 2);
        free(ppre);

        // test aligned_alloc dependent on mpi recv
        void* ptraa = aligned_alloc(recv_size, recv_size * 4);
        free(ptraa);
        
        // not dependent on mpi recv
        int ierr3 = 3;
