...
typedef struct {
  ...
  float resid;        /* desired residual - 
			 normalized as sqrt(r*r)/sqrt(src_e*src_e */
  float size_r;       /* resulting residual */
...
typedef struct {
  float Kappa;        /* hopping */
  float Clov_c;       /* Perturbative clover coeff */
  float U0;           /* Tadpole correction to Clov_c */
...
typedef struct {
  ...
  float r0;           /* source size for w_source */
