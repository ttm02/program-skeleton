...
void dump_info() 
   {
     fprintf(fp,"squaresize %d %d %d %d nsquares %d %d %d %d \n ", squaresize[0],squaresize[1],squaresize[2],squaresize[3],nsquares[0],nsquares[1],nsquares[2],nsquares[3]);
     fflush(fp);
   }

void fixup_ranks(int n) {
  MPI_Comm  spincomm, workcomm;
  int rank, size, ssize, newsize;
  ...
  if (n < size ) {
       if (rank  >= n)  {
          MPI_Comm_split(MPI_COMM_WORLD, 1, 0, &spincomm);
          MPI_Finalize();
          exit(0);
       }  else {
          MPI_Comm_split(MPI_COMM_WORLD, 0 ,0, &workcomm);
       }
       comm = workcomm;
  }
  ...
  number_of_nodes = numnodes();
   ...
   {
     fp =fopen("traceranks","a");
     fprintf(fp,"Using %d number nodes reduced from %d\n ", number_of_nodes,size); 
     fflush(fp);
   }

  if (number_of_nodes != n) {
    printf("supreme failure %d %d \n", number_of_nodes, n);
    exit(1);
  }
  if(flag) err_func(&comm, &flag);
  flag = MPI_Errhandler_create(err_func, &errhandler);
  if(flag) err_func(&comm, &flag);
  flag = MPI_Errhandler_set(comm, errhandler);
  if(flag) err_func(&comm, &flag);
...
/*
 * this portion of the code seems to be broke, and it
 * seems to work if we just skip the call all together.
 *
  MPI_Attr_get(comm, MPI_TAG_UB, &tag_ub, &flag);
...
{
  float work;
  MPI_Allreduce( fpt, &work, 1, MPI_FLOAT, MPI_SUM, comm );
  *fpt = work;
}
...
g_vecfloatsum(float *fpt, int nfloats)
{
  ...
  MPI_Allreduce( fpt, work, nfloats, MPI_FLOAT, MPI_SUM, comm );
...
{
  double work;
  MPI_Allreduce( dpt, &work, 1, MPI_DOUBLE, MPI_SUM, comm );
  *dpt = work;
}
...
g_vecdoublesum(double *dpt, int ndoubles)
{
  ...
  MPI_Allreduce( dpt, work, ndoubles, MPI_DOUBLE, MPI_SUM, comm );
...
{
  complex work;
  MPI_Allreduce( cpt, &work, 2, MPI_FLOAT, MPI_SUM, comm );
  *cpt = work;
}
...
g_veccomplexsum(complex *cpt, int ncomplex)
{
  ...
  MPI_Allreduce( cpt, work, 2*ncomplex, MPI_FLOAT, MPI_SUM, comm );
...
{
  double_complex work;
  MPI_Allreduce( cpt, &work, 2, MPI_DOUBLE, MPI_SUM, comm );
  *cpt = work;
}
...
g_vecdcomplexsum(double_complex *cpt, int ncomplex)
{
  ...
  MPI_Allreduce( cpt, work, 2*ncomplex, MPI_DOUBLE, MPI_SUM, comm );
  for(i=0; i<ncomplex; i++) cpt[i] = work[i];
  free(work);
}
...
{
  float work;
  MPI_Allreduce( fpt, &work, 1, MPI_FLOAT, MPI_MAX, comm );
  *fpt = work;
}
...
{
  double work;
  MPI_Allreduce( dpt, &work, 1, MPI_DOUBLE, MPI_MAX, comm );
  *dpt = work;
}
...
{
  MPI_Bcast( fpt, 1, MPI_FLOAT, 0, comm );
}
...
{
  MPI_Bcast( dpt, 1, MPI_DOUBLE, 0, comm );
}
...
{
  MPI_Bcast( cpt, 2, MPI_FLOAT, 0, comm );
}
...
{
  MPI_Bcast( cpt, 2, MPI_DOUBLE, 0, comm );
}
...
{
  MPI_Send( address, 1, MPI_INT, tonode, SEND_INTEGER_ID, comm );
}
...
{
  MPI_Send( buf, size, MPI_BYTE, tonode, SEND_FIELD_ID, comm );
}
...
{
  MPI_Status status;
  MPI_Recv( buf, size, MPI_BYTE, fromnode, SEND_FIELD_ID, comm,
	    &status );
}
  ...
  int *n_msgs)          /* returns number of messages in list */
{
  ...
  tsiz = numnodes()*sizeof(comlink *);
  combuf = (comlink **)malloc( tsiz); 
    if (combuf==NULL)
    {printf("malloc failed %s %s\n",__FILE__,__LINE__);
    terminate(1);
  } 
  ...
  *n_msgs = 0;
  ...
  for(j=0; j<numnodes(); j++) {
    ...
    *comptpt = compt;
    ...
    if (compt->sitelist[NUM_SUBL]==NULL)
    {printf("malloc failed %s %s\n",__FILE__,__LINE__);
    terminate(1);
  } 
  ...
  comlink *send)       /* neighborlist_send */
{
  ...
  for(i=0; recv!=NULL; ++i, recv=recv->nextcomlink) {
    buf[i] = i;
    MPI_Isend( &buf[i], 1, MPI_INT, recv->othernode, 0, comm,
	       &req[i] );
  }
  ...
  while(send!=NULL) {
    ...
    MPI_Irecv( &i, 1, MPI_INT, send->othernode, 0, comm, &sreq );
    MPI_Wait( &sreq, &stat );
    tol->id_offset = i;
    tol_next = &(tol->next);
...
{
  int work;
  MPI_Allreduce( &n_recv, &work, 1, MPI_INT, MPI_MAX, comm );
  return work;
}
...
do_gather(msg_tag *mtag)  /* previously returned by start_gather */
{
  ...
  for(i=0; i<mtag->nrecvs; i++) {
    ...
    MPI_Irecv( mbuf[i].msg_buf, mbuf[i].msg_size, MPI_BYTE, MPI_ANY_SOURCE,
	       GATHER_ID(mtag->ids[mbuf[i].id_offset]), comm,
	       &mbuf[i].msg_req );
  }
