...
int step, iters=0;
double final_rsq;
void predict_next_xxx(float *oldtime,float *newtime,float *nexttime);
float cg_time,old_cg_time,next_cg_time;	/* simulation time for last two CG's */
#ifdef HMC_ALGORITHM
...
/*ENDDEBUG*/
	...
	next_cg_time = ((float)step-0.5)*epsilon;
	...
	/* update U's to middle of interval */
        ...
        /* do conjugate gradient to get (Madj M)inverse * phi */
     	iters += ks_congrad( F_OFFSET(phi), F_OFFSET(xxx), mass, 
			     niter, rsqmin, EVEN, &final_rsq );
	cg_time = ((float)step - 0.5)*epsilon;
