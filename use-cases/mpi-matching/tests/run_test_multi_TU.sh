...
cmake -DCMAKE_C_COMPILER=clang -DCMAKE_C_FLAGS="-flto -fuse-ld=lld -fwhole-program-vtables -O2" -DCMAKE_CXX_COMPILER=clang++ -DCMAKE_CXX_FLAGS="-flto -fuse-ld=lld -fwhole-program-vtables -O2" $TEST_DIR
make
...
cmake -DCMAKE_C_COMPILER="${MPICC}" -DCMAKE_C_FLAGS="-flto -fuse-ld=lld -fwhole-program-vtables -O2" -DCMAKE_CXX_COMPILER="${MPICXX}" -DCMAKE_CXX_FLAGS="-flto -fuse-ld=lld -fwhole-program-vtables -O2" $TEST_DIR
